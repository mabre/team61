...
public class Terrain extends GridPane {
     ...
     * @param canWalkThroughFigures when true, the object is able to walk through figures (no CollisionException will be thrown) TODO therefore, have a wrapper function which does not throw this exception
     ...
     */
    public Point2D getPositionForDirection(Point2D oldPosition, Point2D direction, Rectangle2D hitRegion, boolean canWalkAlongDiagonals, boolean canWalkThroughFigures, boolean hasMass, boolean snapToPx) throws CollisionException {
                    ...
                    for (int x = minX; x <= maxX && !triedDiagonal; x++) {
                        ...
                        if (intersects) {
                            ...
                            if (canWalkAlongDiagonals && tries == 0 && intersectingFigure == null) {
                                ...
                                } else { throw new CollisionException("figure",collidingPosition, newPosition);}
                            }
                        }
                    }
                } // for each field
