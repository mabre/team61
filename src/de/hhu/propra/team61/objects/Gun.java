...
public class Gun extends Weapon { //ToDo rename to a more fitting one
    private static final int SPEED = 20; // TODO speed should be determined by user
    ...
    public Projectile shoot() throws NoMunitionException {
        ...
        if(munition > 0) {
            ...
            Projectile shot = new Projectile(image, new Point2D(getTranslateX()+offset, getTranslateY()+offset), new Point2D(getCrosshair().getTranslateX()+offset, getCrosshair().getTranslateY()+offset), SPEED, MASS, getDamage());
            munition--;
